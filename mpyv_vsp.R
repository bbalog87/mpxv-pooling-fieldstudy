...
# Normalize coverage using Z-score normalization for each Run-Sample combination
coverage_data_normalized <- coverage_data_filtered %>%
  group_by(Run, Sample) %>%
  mutate(Z_Score_Depth = (Depth - mean(Depth)) / sd(Depth))

# Plot Z-score normalized coverage
ggplot(coverage_data_normalized, 
       aes(x = Position, y = Z_Score_Depth, color = Run, group = Run)) +
  geom_line(alpha = 0.8, size = 1) +
  facet_wrap(~ Sample, scales = "free_x") +
  theme_minimal() +
  labs(title = "Z-Score Normalized Depth Coverage for Samples S1, S7, and S13 Across Runs",
       x = "Genomic Position",
       y = "Z-Score Normalized Coverage",
       color = "Run") +
  scale_color_manual(values = c("RUN1" = "#1f78b4", "RUN2" = "#33a02c", "RUN3" = "#e31a1c"))  # Custom colors for clear distinction
...
ggplot(coverage_data_binned_normalized, aes(x = Bin_Start, y = Z_Score_Coverage, color = Run, group = Run)) +
  ...
  facet_wrap(~ Sample, scales = "free_x") +
  theme_minimal() +
  labs(title = "Z-Score Normalized 5 kb Binned Depth Coverage for Samples S1, S7, and S13 Across Runs",
...
ggplot(coverage_data_binned_normalized, aes(x = Bin_Start, y = Z_Score_Coverage, color = Run, group = Run)) +
  geom_line(alpha = 0.8, size = 1) +
  facet_wrap(~ Sample, scales = "free_x", labeller = label_both) +
  theme_minimal() +
  theme(
    strip.text = element_text(face = "bold", size = 20),  # Make sample names bold and adjust font size
    plot.title = element_text(hjust = 0.5, size = 22, face = "bold"),  # Center title and make bold with size 22
    axis.title = element_text(size = 20),  # Font size for axis titles
    axis.text = element_text(size = 20),  # Font size for axis text
    legend.text = element_text(size = 22, face = "bold"),  # Bold legend text and adjust font size
    legend.title = element_text(size = 22, face = "bold"),  # Bold legend title and adjust font size
    plot.margin = margin(10, 10, 20, 10)  # Add space between x-axis title and plotting area
  ) +
  scale_x_continuous(breaks = seq(0, max(coverage_data_binned_normalized$Bin_Start), by = 50000),  # Custom x-axis ticks at 50 kb intervals
                     labels = function(x) paste0(x / 1000, " kb")) +  # Format labels as "50 kb", "100 kb", etc.
  labs(
    title = "Z-Score Normalized 5 kb Binned Depth ",
    x = "Genomic Position (Bin Start)",
    y = "Z-Score Normalized Average Depth (5 kb Bins)",
    color = "Run"
  ) +
  scale_color_manual(values = c("RUN2" = "#1f78b4",
                                "RUN3" = "#33a02c", 
...
ggplot(coverage_data_binned_normalized, aes(x = Bin_Start, y = Z_Score_Coverage, color = Run, group = Run)) +
  ...
  theme(
    strip.text = element_text(face = "bold", size = 20),  # Make sample names bold and adjust font size
    plot.title = element_text(hjust = 0.5, size = 22, face = "bold"),  # Center title and make bold with size 22
    axis.title.x = element_text(size = 20, margin = margin(t = 15)),  # Add space between x-axis title and tick labels
    axis.title.y = element_text(size = 20),  # Font size for y-axis title
    axis.text = element_text(size = 20),  # Font size for axis text
    legend.text = element_text(size = 22, face = "bold"),  # Bold legend text and adjust font size
    legend.title = element_text(size = 22, face = "bold"),  # Bold legend title and adjust font size
    plot.margin = margin(10, 10, 10, 10)  # Keep default plot margins
  ) +
  scale_x_continuous(
    breaks = seq(0, 200000, by = 50000),  # Custom x-axis ticks at 50 kb intervals, up to 200 kb
    labels = function(x) paste0(x / 1000, "kb")  # Format labels as "50 kb", "100 kb", etc.
  ) +
  labs(
    title = "Z-Score Normalized 5 kb Binned Depth",
...
ggplot(coverage_data_binned_normalized, aes(x = Bin_Start, y = Run, fill = Z_Score_Coverage)) +
  geom_tile() +
  facet_wrap(~ Sample, scales = "free_x") +
  ...
  labs(
    title = "Heatmap of Z-Score Normalized Coverage Across Genomic Bins",
    x = "Genomic Position (Bin Start)",
    y = "Run"
  )
...
# Plot heatmap of Z-score normalized coverage
ggplot(coverage_data_binned_normalized, aes(x = Bin_Start, y = Run, fill = Z_Score_Coverage)) +
  geom_tile() +
  facet_wrap(~ Sample, scales = "free_x") +
  scale_fill_viridis(option = "C", name = "Z-Score", direction = -1) +  # Colorblind-friendly and high-contrast
  theme_minimal() +
  theme(
    strip.text = element_text(face = "bold", size = 16),
...
ggplot(coverage_data_binned_normalized, aes(x = Bin_Start, y = Run, fill = Z_Score_Coverage)) +
  ...
  scale_fill_viridis(option = "C", name = "Z-Score", direction = -1) +  # Colorblind-friendly and high-contrast
  scale_x_continuous(
    breaks = seq(0, max(coverage_data_binned_normalized$Bin_Start), by = 50000),  # Custom x-axis ticks at 50 kb intervals
    labels = function(x) paste0(x / 1000, "kb")  # Format labels as "50 kb", "100 kb", etc.
  ) +
  theme_minimal() +
  theme(
  ...
  labs(
    title = "Heatmap of Z-Score Normalized Depth Across Genomic Bins",
    x = "Genomic Position (Bin Start)",
    y = "Run"
  )
